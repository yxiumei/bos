...
<script type="text/javascript">
	...
	function doDelete() {
		...
		if(rows.length !=0){
			$.messager.confirm("确认删除","您确定要删除吗？", function(flg) {
				// 创建一个数组
				var array = new Array();
				if (flg){
					...
					// 拼接成字符串
					var ids = array.join(",");
					location.href="${pageContext.request.contextPath}/regionAction_delBatch?ids="+ids;
				}
			});
		}else{
			$.messager.alert("提示","请选择一个区域！","warning");
		}
	}
	...
	$(function() {
		...
		// 页面加载完成，区域表格创建完成后，调用插件upload方法，动态修改HTML元素
		$("#button-import")
				.upload(
						{
							action : '${pageContext.request.contextPath}/regionAction_fileUpload',
							name : 'uploadExl'
						});
	...
	function doDblClickRow(rowIndex, rowData) {
		...
		// 数据进行回写
		$("#editregionFrom").form("load", rowData); //rowData json对象的Region数据
	}
</script>
</head>
<body class="easyui-layout" style="visibility:hidden;">
	...
	<!-- 修改区域页面 -->
	<div class="easyui-window" title="区域修改" id="editRegionWindow"
		collapsible="false" minimizable="false" maximizable="false"
		style="top:20px;left:200px">
		<div region="north" style="height:31px;overflow:hidden;" split="false"
			border="false">
		...
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form id="editregionFrom"
				action="${pageContext.request.contextPath }/regionAction_editRegion"
				method="post">
				<!-- 编辑取区域的id -->
