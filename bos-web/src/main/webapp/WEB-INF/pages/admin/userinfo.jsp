       ...
       <form id="form" method="post" action="${pageContext.request.contextPath }/UserAction_add">
           <table class="table-edit"  width="95%" align="center">
				...
				<tr>
	           	...
	           	<tr>
	           		<td>选择角色:</td>
	           		<td colspan="3" id="TDRole">
	           		<script type="text/javascript">
	           			$(function(){
	           				// 页面加载完成，显示角色选择
	           				$.post(
	           					'${pageContext.request.contextPath}/roleAction_listAjax',
	           					function(data){
	           						for (var i = 0;i < data.length;i++){
	           							var id = data[i].id;
	           							var name = data[i].name;
	           							$("#TDRole").append("<input type='checkbox' name='roles' id='"+id+"' value='"+id+"'>"+"<label for='"+id+"'>"+name+"</label>");
	           						}
	           					}
	           				);
	           			});
